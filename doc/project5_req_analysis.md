...
| 1.1 | 2026-05-18 | 김지현 | 순차 다이어그램 예외 흐름 추가, 비기능 요구사항 추적표 반영 | 팀원 코멘트 반영 |
...
> **변경사항:** 유스케이스 명세(U_03) 예외 흐름 2.a1 반영 — 지원하지 않는 파일 형식 또는 용량 초과 시 실패 처리 추가
...
> **변경사항:** 유스케이스 명세(U_09) 예외 흐름 3.a1 반영 — 검색 결과 없음 처리 추가
...
> **변경사항:** 비기능 요구사항 추적을 위해 각 항목에 NFR ID를 부여하였다.
...
> **변경사항:** 비기능 요구사항(NFR) 항목을 추적표에 추가하여 기능 요구사항과 함께 관리한다.
...
|--------|------|--------------------------|-----------|
| NFR-001 | 운영 환경 | 전체 (U_01 ~ U_13) | 브라우저 호환성 테스트 |
| NFR-002 | 성능 | U_03 (업로드), U_04 (다운로드) | 100MB 파일 업/다운 응답 시간 측정 |
| NFR-003 | 성능 | U_09 (검색) | 검색 응답 시간 측정 (3초 이내) |
| NFR-004 | 동시 접속 | 전체 (U_01 ~ U_13) | 부하 테스트 (동시 사용자 200명) |
| NFR-005 | 보안 | U_01 (회원가입), U_02 (로그인) | 비밀번호 암호화 저장 여부 확인 |
| NFR-006 | 보안 | U_04 (다운로드), U_07 (공유), U_08 (링크) | 권한 없는 접근 시도 차단 확인 |
| NFR-007 | 사용성 | 전체 (U_01 ~ U_13) | 화면 이동 단계 수 측정 (3단계 이내) |
| NFR-008 | 유지보수성 | 전체 (U_01 ~ U_13) | 모듈 구조 코드 리뷰 |
